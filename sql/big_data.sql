-- Jeu de données conséquent (~300k lignes) pour démonstrations et benchmarks
-- Inclut plusieurs communes

CREATE SCHEMA IF NOT EXISTS referentiels;

-- S'assure que la table existe (au cas où le bootstrap n'a pas été lancé)
...
-- Insertion en masse via generate_series
WITH base AS (
    SELECT
        (1 + (random()*999)::int) AS numero,
        CASE 
            WHEN random() < 0.02 THEN 'rue des jacinthes'
            WHEN random() < 0.05 THEN 'avenue de la République'
